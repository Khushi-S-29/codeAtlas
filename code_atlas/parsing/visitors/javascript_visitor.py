...
class JavaScriptVisitor(BaseVisitor):
    ...
    def parse(self, source_code: str, file_path: str, repo_id: str) -> ParseResult:
        ...
        for call in self.find_all(root, "call_expression"):
            callee = self._callee_name(call, source_bytes)
            if self._is_require_call(call):
                target = self._require_target(call, source_bytes)
                if target:
            ...
            if args:
                for ident in self.find_all(args, "identifier"):
                    module_refs.append(self.node_text(ident, source_bytes).strip())
