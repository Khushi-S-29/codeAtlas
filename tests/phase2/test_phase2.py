...
class TestGoVisitor:
    ...
    def _parse(self, code: str, path: str = "test.go") -> object:
        from code_atlas.parsing.visitors.go_visitor import GoVisitor
        return GoVisitor().parse(code, path, "test-repo")

    def test_extracts_module_node_with_package_name(self):
        code = "package auth\n"
        result = self._parse(code)
        modules = [n for n in result.nodes if n.kind == NodeKind.MODULE]
        assert len(modules) == 1
        assert "auth" in modules[0].name

    def test_extracts_function(self):
        code = "func Calculate(a int, b int) int { return a + b }\n"
        result = self._parse(code)
        funcs = [n for n in result.nodes if n.kind == NodeKind.FUNCTION]
        assert len(funcs) == 1
        assert funcs[0].name == "Calculate"
        assert "a" in funcs[0].parameters
    ...
    def test_extracts_method_with_receiver(self):
        code = "func (u *User) Save() error { return nil }\n"
        result = self._parse(code)
        methods = [n for n in result.nodes if n.kind == NodeKind.METHOD]
        assert len(methods) == 1
    ...
    def test_extracts_imports_standalone_and_block(self):
        code = (
            'import "fmt"\n'
            'import (\n'
            '    "os"\n'
        ...
        assert any('"net/http"' in name for name in names)
    ...
    def test_extracts_call_expressions(self):
        ...
        funcs = [n for n in result.nodes if n.kind == NodeKind.FUNCTION]
        calls = funcs[0].calls
        assert any("fmt.Println" in c for c in calls)
        assert any("http.Get" in c for c in calls)

    def test_private_function_not_exported(self):
        code = "func internalHelper() {}"
        result = self._parse(code)
        funcs = [n for n in result.nodes if n.kind == NodeKind.FUNCTION]
        assert funcs[0].is_exported is False
